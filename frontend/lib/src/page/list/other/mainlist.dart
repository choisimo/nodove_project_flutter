...
class _FeedMainListState extends State<FeedMainList> {
  ...
  Widget build(BuildContext context) {
    NavbarContent navbarOpt =
        NavbarContent(title: const NavbarTitle("홈"), actions: [
      NavbarCommonBtn(
        "navbar/alert-empty.svg",
        onClick: () => Navigator.of(context)
            .push(MaterialPageRoute(builder: (_) => const NotiPage())),
      )
    ]);
    ...
    return CustomScrollView(
        ...
        slivers: [
          ...
          SliverToBoxAdapter(child: MainTagList(tagList: tagList)),
    ...
    return PartContainer(children: [
      ...
      Container(
          ...
          child: PageView(
            ...
            children: con.comm.map((e) => FeedContent(feed: e[0])).toList(),
          ))
    ]);
  }
}
...
class TitleRow extends StatelessWidget {
  ...
  const TitleRow({super.key, this.title, this.fontSize = 18, this.onTap});
